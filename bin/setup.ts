...
program
  ...
  .description('Configure @blockchain-forever/aelf-node-skill for Claude/Cursor/OpenClaw/IronClaw')
...
const withCommonMcpOptions = (command: Command) =>
  command
    ...
    .option('--force', 'Overwrite existing aelf-node-skill entry', false);
...
program
  ...
  .action(() => {
    ...
    console.log(`    Config file: ${claudeExists ? 'exists' : 'not found'} | ${SERVER_NAME}: ${claudeConfigured ? 'configured' : 'not configured'}`);
    ...
    console.log(`    Config file: ${cursorGlobalExists ? 'exists' : 'not found'} | ${SERVER_NAME}: ${cursorGlobalConfigured ? 'configured' : 'not configured'}`);
    ...
    console.log(`    Config file: ${cursorProjectExists ? 'exists' : 'not found'} | ${SERVER_NAME}: ${cursorProjectConfigured ? 'configured' : 'not configured'}`);
...
program
  ...
  .description('Remove aelf-node-skill setup from platform (claude|cursor|ironclaw|openclaw)')
