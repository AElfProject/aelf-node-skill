...
export function setupOpenClaw(opts: { configPath?: string; cwd?: string; force?: boolean }): boolean {
  ...
  if (!opts.configPath) {
    const outputPath = path.join(process.cwd(), 'aelf-node-openclaw.json');
...
export function uninstallOpenClaw(opts: { configPath?: string }): boolean {
  ...
  if (removed <= 0) {
    LOG.info('No aelf-node-skill tools found in target OpenClaw config.');
    return false;
  }

  writeJsonFile(opts.configPath, existing);
  LOG.success(`Removed ${removed} aelf-node-skill tools from OpenClaw config.`);
  return true;
}
