...
describe('MCP raw annotation contract', () => {
  ...
  test('raw tools/list includes both camelCase and snake_case approval hints', async () => {
    ...
    const readOnly = tools.find(tool => tool.name === 'aelf_get_chain_status');
    const localWrite = tools.find(tool => tool.name === 'aelf_import_node');
    const networkWrite = tools.find(tool => tool.name === 'aelf_send_contract_transaction');
