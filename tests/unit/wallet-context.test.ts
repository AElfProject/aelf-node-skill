...
describe('wallet context storage', () => {
  ...
  beforeEach(() => {
    tempDir = join(tmpdir(), `aelf-node-wallet-context-${Date.now()}-${Math.random()}`);
  ...
  test('setActiveWalletProfile creates directory and context file', () => {
    ...
    expect(readWalletContext()?.profiles.default?.address).toBe('ELF_node_AELF');
  });
